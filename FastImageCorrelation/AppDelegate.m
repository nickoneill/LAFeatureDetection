...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
    ...
    if ([points count] > 8) {
        NSLog(@"more than 8 possible points is an unlikely match");
    }
    ...
    for (int i = 0; i < kernel.size.width+1; i++) {
        [sampleRep setColor:red atX:(primary.y-i) y:primary.x];
        [sampleRep setColor:red atX:(primary.y-i) y:(primary.x-kernel.size.height)];
    }
    for (int i = 0; i < kernel.size.height; i++) {
        [sampleRep setColor:red atX:primary.y y:(primary.x-i)];
        [sampleRep setColor:red atX:(primary.y-kernel.size.width) y:(primary.x-i)];
    }
